...
function setupSchema() {
  ...
  Object.keys(SHEETS_).forEach(function(name){
    ...
    sh.setFrozenRows(1); sh.getRange(1,1,1,headers.length).setFontWeight('bold').setBackground('#172c38').setFontColor('#ffffff');
  });
...
function dispatch_(action,payload,actor,cfg,requestId) {
  ...
  if(action==='admin.report-payment-from-chat') return recordChatPayment_(payload,actor,requestId);
...
function startMachine_(p,actor,requestId,stage) {
  ...
  var openMachineTimer=table_('Timers').find(function(t){return t.machine_id===machine.machine_id&&!t.completed_at;});
  if(machine.current_order_id||openMachineTimer)throw apiError_('MACHINE_BUSY','เครื่องนี้ยังมีงานที่กำลังใช้งานหรือยังไม่ปิดเวลา กรุณารีเฟรชคิวก่อนเลือกเครื่อง',409);
