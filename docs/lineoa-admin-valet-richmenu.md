...
- The Cloudflare Worker is staging-only and `/health` reports `mode: demo`, version `516f70c`. The authorized secret names are configured in Cloudflare, including the correct LINE OA token and admin-only recipient. Secret values are intentionally not recorded here. `GAS_BACKEND_URL` is not configured because there is no verified WDF web-app deployment.
